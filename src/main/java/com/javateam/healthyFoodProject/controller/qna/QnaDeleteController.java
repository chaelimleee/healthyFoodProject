...
public class QnaDeleteController {
	...
	public String updateProc(@RequestParam("qnaCode") int qnaCode, 
							 @RequestParam("memberEmail") String memberEmail,
							//@RequestParam("qnaPass") String qnaPass,
						     Model model) {

		log.info("------ deleteProc.do : qnaCode : {} , memberEmail : {} ", qnaCode, memberEmail );
		...
		QnaVO qnaVO = qnaService.selectQna(qnaCode); // 기존 정보 읽어오기
		
		log.info("기존 정보 : qnaVO : {}", qnaVO);
		
		// 댓글들 현황 점검
		// 댓글들이 있다면 삭제 못하도록 차단 !
		// 댓글이 없을 경우에 삭제 허용 !
		if (qnaService.selectQnasCountWithReplies(qnaCode) > 0) { // 댓글(들)이 있을 경우
			...
			//log.info("전송 패쓰워드 : {}", qnaPass);
			//log.info("DB 패쓰워드 : {}", qnaVO.getQnaPass());
			
			// 게시글 패쓰워드 검증 song 수정안됨 0412 qnaQnaVO-->qnaVO로 바꾸고 싶음 
			if (memberEmail.trim().equals(qnaVO.getMemberEmail())) {
				
				log.info("이메일 점검 성공");
			
				// 삭제할 삽입 이미지 점검
				List<Integer> deleteImgList = qnaService.getImageList(qnaVO.getQnaContent().trim(),
						"/qna/image/");
	...
	////////////////////////////////////////////////
				...
				// 첨부 파일 삭제
				if (qnaVO.getQnaOriginalFile() != null) { // 첨부 파일이 있다면
					
					log.info("첨부 파일 존재 !");

					// 기존 첨부 파일 삭제
					msg += fileUploadService.deleteUploadFile(qnaVO.getQnaFile());
					log.info("파일 삭제 msg : {}", msg);

				} // if 
				
				// 게시글 테이블(board_tbl)에서 게시글 자체를 삭제
				if (qnaService.deleteById(qnaCode) == true) {
					
					msg = "게시글 삭제에 성공하였습니다.";
					movePage = "/qna/list.do"; // 게시글 목록으로 인동
			...
			}  // if (boardPass.trim().equals(boardVO.getQnaPass()))	
			
		
		} // (boardService.selectQnasCountWithReplies(qnaCode) > 0) 
